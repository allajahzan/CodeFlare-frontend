...
function DropDown({ selectedItem, stateVariable, setStateVariable1, setStateVariable2, setStateVariable3, handleFunction, datas }: propType) {
    return (
        <div className="relative">
            <div onClick={() => { setStateVariable1(!stateVariable); setStateVariable2?.(false); setStateVariable3?.(false) }} className="p-3 bg-white flex items-center justify-between bg-transparent border-2 border-black border-opacity-20 font-medium rounded-lg cursor-pointer">
                <p style={{ fontSize: '14.5px' }}>{selectedItem}</p>
                <img className="w-6" src={stateVariable ? upArrow : downArrow} alt="" />
            </div>

            {stateVariable && (
                <div className={`absolute z-50 left-0 right-0 ${(datas as string[]).length > 5 ? 'h-[300px]' : ''} overflow-y-auto mt-1.5 bg-white border border-gray-200 rounded-lg shadow-lg`}>
